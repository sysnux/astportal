...
def sip_update():
   ...
   try:
      file = open('/etc/asterisk/INC/sip-inc.conf','w')
...
''' % (friend.number, friend.password, context, callgroups, pickupgroups, friend.number, cidname, int(friend.number))
...
def voicemail_update():
   ...
   try:
      file = open('/etc/asterisk/INC/voicemail-inc.conf','w')
      file.write('; Automatically generated by astportal, manual changes will be lost!\n')
...
class Phone_ctrl(RestController):
   ...
   def grandstream_configure(self, ip, mac):
      ...
      gs.pre_configure(server, server, server,
            server + ':8080/phonebook/gs_phonebook_xml', server)
   ...
   def create(self, **kw):
      ...
      if kw['mac']:
         ...
         new_phone.configure( pwd, server + '/phones/firmware', 
            ...
            server + ':8080/phonebook/gs_phonebook_xml', '', '', '',
